@section('content')
    ...
    <div class="chat_container">
        ...
        <div class="active_field">
            <div class="chat">
                <div class="pinned @if (!$chat_data['pinned']) hidden @endif ">
                        <p class="pin_t">Закрепленное сообщение <a href="#">Открепить</a></p>
                        <div class="mes">
                            <span class="user">@if ($chat_data['pinned'])
                                    {{ $chat_data['pinned']->nickname }}
                                @endif
                            </span>
                            <span class="message">: @if ($chat_data['pinned'])
                                    {{ $chat_data['pinned']->message }}
                                @endif
                            </span>
                            <p class="date">@if ($chat_data['pinned'])
                                    {{ $chat_data['pinned']->date }}
                                @endif
                            </p>
                        </div>
                    </div>
