            ...
            @foreach($users as $user_info)
                <tr class="truser" id="{{ $user_info->id }}">
                    ...
                    @if(!($user_info->is_admin || $user_info->id == \Illuminate\Support\Facades\Auth::id()))
                        <td><button class="button middle editUser">Изменить</button></td>
                        <td><button class="button middle blockUser">@if($user_info->status == 'banned')Разблокировать@elseЗаблокировать@endif</button></td>
                        <td><button class="button middle deleteUser">Удалить</button></td>
                    @endif
                </tr>
            @endforeach
        </table>
